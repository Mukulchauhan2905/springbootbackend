...
public class OrderServiceImpl implements OrderService {
	...
	public void orderPlace(String customer, String price) {
		...
		for (Address a : address) {
			if (a.getCustomerEmail().equals(customer)) {
				...
				tempAdd.setPincode(a.getPincode());
		...
		for (Cart c : cart) {
			if (c.getCustomerEmail().equals(customer)) {
				...
				p.setPincode(tempAdd.getPincode());
	...
	public PlacedOrders updateOrderStatus(int id, int status) {
		PlacedOrders placeodr = orderRepo.findById(id)
		.orElseThrow(() -> new ProductCustomException("Product Item not found"));
		if(status==1) {
			placeodr.setOrderStatus("Confirmed");
		}
		else if(status==0) {
			placeodr.setOrderStatus("Sorry Order Cancelled");
		}
		orderRepo.save(placeodr);
		return placeodr;
